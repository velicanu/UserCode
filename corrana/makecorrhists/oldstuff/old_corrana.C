...
TH2D * JetTrackSignal(double pttriglow , double pttrighigh , double ptasslow , double ptasshigh, double vzrange);
...
TH2D * JetTrackSignal(double pttriglow , double pttrighigh , double ptasslow , double ptasshigh, double vzrange)
{
  Long64_t nentries = c->GetEntries();
  Long64_t nbytes = 0, nb = 0;
  TH2D * hJetTrackSignal = new TH2D("hJetTrackSignal",";#Delta#eta;#Delta#phi",50,-5,5,75,-pi,2*pi);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    int ntrig = 0 , nass = 0;
    vector<double> trigjetEta;
    vector<double> trigjetPhi;
    vector<double> asstrkEta;
    vector<double> asstrkPhi;
    vector<double> asstrkIndex;
    if(c->icPu5.nref>0 && c->icPu5.jtpt[0] > pttriglow && c->icPu5.jtpt[0] < pttrighigh )
    {
      ++ntrig;
      trigjetEta.push_back(c->icPu5.jteta[0]);
      trigjetPhi.push_back(c->icPu5.jtphi[0]);
    }
    for(int j = 0 ; j < c->track.nTrk ; ++j)
    {
      if( c->track.trkPt[j]<ptasslow || c->track.trkPt[j]>ptasshigh) continue;
      ++nass;
      asstrkEta.push_back(c->track.trkEta[j]);
      asstrkPhi.push_back(c->track.trkPhi[j]);
      asstrkIndex.push_back(j);
    ...
    for(int i = 0 ; i < ntrig ; ++i)
    {
      ...
      for(int j = 0 ; j < nass ; ++j)
      {
        double deta = fabs(trigjetEta[i]-asstrkEta[j]);
        double dphi = fabs(trigjetPhi[i]-asstrkPhi[j]);
        if( dphi > pi ) dphi = 2*pi - dphi;
...
TH2D * JetTrackBackground(double pttriglow , double pttrighigh , double ptasslow , double ptasshigh, double vzrange, int statfactor, double vzbin)
{
  Long64_t nentries = c->GetEntries();
  Long64_t nbytes = 0, nb = 0;
  TH2D * hJetTrackBackground = new TH2D("hJetTrackBackground",";#Delta#eta;#Delta#phi",50,-5,5,75,-pi,2*pi);
  
  
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    c->GetEntry(jentry);
    cout<<jentry<<"/"<<1000<<endl;
    
    if(fabs(c->evt.vz)>vzrange) continue;
    double thisvz = c->evt.vz;
    int njetinptrange = 0;
    vector<double> thisjetEta;
    vector<double> thisjetPhi;
    if(c->icPu5.nref>0 && c->icPu5.jtpt[0] > pttriglow && c->icPu5.jtpt[0] < pttrighigh )
    {
      ++njetinptrange;
      thisjetEta.push_back(c->icPu5.jteta[0]);
      thisjetPhi.push_back(c->icPu5.jtphi[0]);
    }
    
    int mixentry = jentry;
    int nmix = 0;
    while(nmix < statfactor)
    {
      mixentry++;
      if(mixentry==nentries) mixentry=0;
      c->GetEntry(mixentry);
      
      if(fabs(thisvz-c->evt.vz)>vzbin) continue;
      ++nmix;
      for(int i = 0 ; i < njetinptrange ; ++i)
      {
        for(int j = 0 ; j < c->track.nTrk ; ++j)
        {
          if( c->track.trkPt[j]<ptasslow || c->track.trkPt[j]>ptasshigh) continue;
          double deta = fabs(thisjetEta[i]-c->track.trkEta[j]);
          double dphi = fabs(thisjetPhi[i]-c->track.trkPhi[j]);
          if( dphi > pi ) dphi = 2*pi - dphi;
