...
void mccorrana(const char * infname = "/mnt/hadoop/cms/store/user/velicanu/mergedv1_sortedforest/mergesortv1_0.root")
{
...
TH2D * JetTrackSignal(int jetindex, double leadingjetptlow , double leadingjetpthigh , double subleadingjetptlow , double subleadingjetpthigh , double ptasslow , double ptasshigh, int centmin, int centmax, double vzrange, double dijetdphicut)
{
  ...
  for (Long64_t jentry=cent_index_start[centmin]; jentry<cent_index_start[centmax];jentry++) {
    ...
    if( fabs(c->evt->vz) > vzrange ) continue;
    if( c->icPu5->ngen < 2 )         continue;
    double dijetdphi = fabs(c->icPu5->genphi[0] - c->icPu5->genphi[1]);
    if( dijetdphi > pi ) dijetdphi = 2*pi - dijetdphi;
    if( dijetdphi > dijet_dphi_CUT ) continue;
    if( c->icPu5->geneta[0] > jet_eta_CUT || c->icPu5->geneta[1] > jet_eta_CUT ) continue;
    if( c->icPu5->genpt[0] > leadingjetpthigh || c->icPu5->genpt[0] < leadingjetptlow ) continue;
    if( c->icPu5->genpt[1] > subleadingjetpthigh || c->icPu5->genpt[1] < subleadingjetptlow ) continue;
    double aj = ( c->icPu5->genpt[0] - c->icPu5->genpt[1] ) / ( c->icPu5->genpt[0] + c->icPu5->genpt[1] );
    // if( aj < 0.12 ) continue;
    
    double jeteta = c->icPu5->geneta[jetindex];
    double jetphi = c->icPu5->genphi[jetindex];
...
TH2D * JetTrackBackground(int jetindex, double leadingjetptlow , double leadingjetpthigh , double subleadingjetptlow , double subleadingjetpthigh , double ptasslow , double ptasshigh, int centmin, int centmax, double vzrange, double dijetdphicut, int statfactor)
{
  ...
  for (Long64_t jentry=cent_index_start[centmin]; jentry<cent_index_start[centmax];jentry++) {
    ...
    double aj = ( c->icPu5->genpt[0] - c->icPu5->genpt[1] ) / ( c->icPu5->genpt[0] + c->icPu5->genpt[1] );
    // if( aj > 0.12 ) continue;
    double thisjeteta = c->icPu5->geneta[jetindex];
    double thisjetphi = c->icPu5->genphi[jetindex];
