   ...
   currentEvent(0)
{
  ...
  hfTree        = (TTree*) inf->Get("pfTowers/hf");
...
void HiForest::GetEntry(int i)
{
  ...
  if (hasHFTree)       hfTree    ->GetEntry(i);
...
void HiForest::InitTree()
{
   ...
   if (hasHFTree) {
      hfTree->SetName("hf");
      if (tree == 0) tree = hfTree; else tree->AddFriend(hfTree);
      setupHFTree(hfTree,hf);
   }
...
void HiForest::PrintStatus()
{
  ...
  if (hasHFTree)    CheckTree(hfTree,    "HFTree");
...
void HiForest::SetOutputFile(const char *name)
{
  ...
  if (hasHFTree)    AddCloneTree(hfTree,    "pfTowers",              "hf");
