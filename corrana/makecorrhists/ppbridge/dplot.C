...
void dplot(int trkqual = 1, string filename = "" , string tag = "", int nmin = 130, int nmax = 160, float tptmin = 14, float tptmax = 16, float aptmin = 1, float aptmax = 2, string draw = "corr")
{
  ...
  if(draw.compare("corr")==0)
	{
    c1->SaveAs(Form("corr_trg%d_%d_ass%d_%d_cmin0_cmax41.png",(int)tptmin,(int)tptmax,(int)aptmin,(int)aptmax));
  }
	else if(draw.compare("proj")==0)
	{
    c1->SaveAs(Form("proj_trg%d_%d_ass%d_%d_cmin0_cmax41.png",(int)tptmin,(int)tptmax,(int)aptmin,(int)aptmax));
	}
