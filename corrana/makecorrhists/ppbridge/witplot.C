...
void witplot(){
  ...
  // hcorr_phi_weighted->Divide(hbackground_phi);
	  ...
	  int xmn = Sig[imult][ipt]->GetYaxis()->FindBin(-0.2);
	  int xmx = Sig[imult][ipt]->GetYaxis()->FindBin(0.2);
      double nbins = xmx - xmn + 1;
      ProjSig[imult][ipt] = Sig[imult][ipt]->ProjectionX(Form("projsig_nmin%d_nmax%d_ptmin%d_ptmax%d", (int)nlow[imult], (int)nhigh[imult], (int)ptlow[ipt], (int)pthigh[ipt]),xmn,xmx,"e");
      ProjSig[imult][ipt]->Scale(1/nbins);
      ProjBack[imult][ipt] = Back[imult][ipt]->ProjectionX(Form("projback_nmin%d_nmax%d_ptmin%d_ptmax%d", (int)nlow[imult], (int)nhigh[imult], (int)ptlow[ipt], (int)pthigh[ipt]),xmn,xmx,"e");
